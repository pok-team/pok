...
#define pok_thread_sleep_until(time)                                           \
  pok_syscall2(POK_SYSCALL_THREAD_SLEEP_UNTIL, (uint32_t)time, 0)

#define pok_thread_wait_infinite() pok_thread_suspend()

#define pok_thread_suspend()                                                   \
  pok_syscall2(POK_SYSCALL_THREAD_SUSPEND, NULL, NULL)

#define pok_thread_suspend_target(thread_id)                                   \
  pok_syscall2(POK_SYSCALL_THREAD_SUSPEND_TARGET, thread_id, 0)

/*
 * Similar to: pok_ret_t      pok_thread_suspend (void);
 */

#define pok_thread_restart(thread_id)                                          \
  pok_syscall2(POK_SYSCALL_THREAD_RESTART, thread_id, 0)
/*
 * similar to:
